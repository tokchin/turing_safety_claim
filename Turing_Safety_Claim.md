...
| 第14章（サイバーセキュリティ）、第18章（事故対応）、第19章（段階的社会実装） | **未着手**（後続改訂で新規執筆） |
| 第16章（型式指定スキームと責任構造） | **第2版相当ドラフト作成済**（別ドラフト `06_Chapter16_TypeDesignation_draft.md` を参照。第2版本文に統合予定） |
| Appendix A〜D | 初版を残置 |
| Appendix E（ISO・規制対応マップ詳細版） | 別ドラフト `04_Appendix_E_ISO_Mapping_detailed.md` を参照（第2版本文に統合予定） |
| Appendix F〜I | 後続改訂で整備 |
| 衛星文書骨子 | `02_VMC_IF_Specification_draft.md`、`03_Vehicle_Requirements_Specification_draft.md`、`05_Chapter9_HeterogeneousSoC_draft.md`、`06_Chapter16_TypeDesignation_draft.md` を参照 |
