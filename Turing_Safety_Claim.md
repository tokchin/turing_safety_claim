...
| --- | --- |
| **作成日** | 2026年4月29日 |
| **位置づけ** | 安全主張、設計思想、後続証拠の位置づけを示す上位文書 |
...
6.4 ISO 21448 / SOTIFの扱い（本書では別枠で適用しない）
...
本書では、TuringのL4自動運転車両に関する安全論証を構成するため、関連するISO規格を参照する。

ただし、本書は各ISO規格への包括的な適合宣言を目的とするものではない。本書におけるISO規格の役割は、L4自動運転車両の安全性を、通常走行時のE2E AI安全、ハードウェア失陥時安全、多重系ADSによるMRC、モデル更新、運用後監視に分けて整理し、それぞれに対して必要な安全主張、設計根拠、後続証拠を明確化することである。
...
本書では、ISO/TS 5083を安全論証全体の骨格として参照する。詳細な対応表は後続文書で具体化する。

## 6.3 ISO 26262の使い方

ISO 26262は、道路車両の安全関連E/Eシステムを対象とする機能安全の規格である。ISO 26262の枠組みは、E/Eシステムの故障に起因する危険を管理するために用いられる。

Turingの安全論証では、ISO 26262を、ハードウェア失陥安全論証の基礎として用いる。
...
| 共通原因故障 | 多重系が同時に失われるリスクを整理する |

本書では、E2E AIの性能不足と、E/Eシステムの故障を混同しない。カメラフリーズ、SoCハング、電源低下、通信断、制動系故障等は、ISO 26262の考え方に基づき、ハードウェア失陥安全として扱う。

## 6.4 ISO 21448 / SOTIFの扱い（本書では別枠で適用しない）

ISO 21448:2022は、ISO 26262で扱う故障を対象外とし、意図した機能の仕様不足または性能不足に起因するリスクを対象とする規格である。

本書では、ISO 21448を独立した適用規格としては用いない。すなわち、SOTIFのプロセス（機能仕様起点のHazard Analysis、Triggering Conditionの体系的同定、Area 1〜4による既知/未知Unsafe Scenarioの分解、SOTIF V-cycleの工程適用）を、本安全論証の別枠の安全活動として実施しない。

別枠で適用しない理由は、以下のとおりである。

第一に、ISO 21448はTriggering Condition→Hazardous Behaviorという機能分解を前提とするが、画像入力から舵角・加速度を直接出力するE2E AIでは、中間機能を切り出してTriggering Conditionをコンポーネント単位に紐づけることができない。SOTIFのプロセスをE2E AIに無理に適用すると、分解粒度がアーキテクチャと一致せず、安全論証が形骸化する。

第二に、ISO/PAS 8800が、AIシステムを対象として、ロバスト性、安定性、ODD適合性、入力品質感度、未到達分布、残存リスク主張を含む枠組みをAI固有の粒度で提供する。これは、ISO 21448が扱う「故障していないが意図機能の性能不足で危ない」というリスク領域を、E2E AIに対してより適切に扱う。本書では、この領域をISO/PAS 8800で扱う。

第三に、非AI構成要素についても、センサ単体の性能限界はAI入力品質劣化として8800側に、ハード健全性として失陥した場合は26262側に分配する。SOTIFが唯一の適用枠となる対象は、本システムのアーキテクチャでは存在しない。

ISO 21448の概念のうち、ODD境界、Known/Unknown Unsafe Scenario、安全側挙動という語彙は、ISO/TS 5083およびISO/PAS 8800を経由して概念的に継承する。具体的には、ODD境界の定義は7章で、E2E AIの性能限界と残存リスクは6.5節および後続のAI安全論証で、シナリオベース評価は6.6節および後続の証拠文書で扱う。

| **対象** | **本書での扱い** |
| --- | --- |
| E2E AIの性能限界 | ISO/PAS 8800で扱う（6.5節） |
| ODD境界 | ODDの定義（7章）およびISO/PAS 8800のODD適合性で扱う |
| 不確実な場面、未知・稀なシナリオ | ISO/PAS 8800の残存リスク主張、ISO 34502系のシナリオ評価で扱う |
| 安全側挙動 | 通常走行安全、MRC、フェイルオーバーの安全主張（4章、5章）で扱う |
| 非AIセンサの性能限界 | 入力品質劣化として8800、ハード失陥として26262に分配する |

本書の立場として、ISO 21448を「軽く参照」することは行わない。安全論証の重複や帰属の曖昧化を避け、E2E AIの性能不足はISO/PAS 8800、E/E失陥はISO 26262、安全論証の骨格はISO/TS 5083という分担を一貫させる。

## 6.5 ISO/PAS 8800の使い方

ISO/PAS 8800:2024は、AIシステムの安全関連特性を用いて、不合理なリスクがないことを示す安全保証主張を構成するための文書であり、主に機械学習を含むAI手法に焦点を当てている。

Turingの安全論証では、ISO/PAS 8800を、E2E AIの安全論証を構成するためのAI専用フレームとして用いる。

| **対象** | **ISO/PAS 8800の使い方** |
| --- | --- |
| E2E AIの安全主張 | AIが安全関連出力を生成することを前提に安全保証主張を構成する |
| AI出力の不足 | 止まるべき場面で十分な減速を出さない等のリスクを扱う |
| AIの体系的エラー | 学習データ、設計、学習過程、評価過程に由来する弱点を扱う |
| AIの安全関連特性 | ロバスト性、安定性、ODD適合性、入力品質への感度を整理する |
| AIと安全監視 | AI単体ではなく、安全監視と組み合わせて安全主張を作る |

本書では、ISO/PAS 8800を、特定のニューラルネットワーク手法を規定するものとしてではなく、AI要素を含む安全関連システムの安全保証主張を構成する参照枠として用いる。

## 6.6 ISO 34502系の使い方

ISO 34502:2022は、ADSに対するシナリオベース安全評価フレームワークのガイダンスを提供する文書であり、開発段階に適用されるシナリオベース評価プロセスを整理している。

Turingの安全論証では、ISO 34502系を、シナリオベース評価の考え方を整理するために用いる。

ただし、本書では具体的な試験条件、合格基準、検証手順は定義しない。これらは後続の証拠文書で具体化する。

| **対象** | **本書での扱い** |
| --- | --- |
| E2E AI評価シナリオ | 後続証拠文書で具体化する |
| MRC評価シナリオ | 後続証拠文書で具体化する |
| フェイルオーバー評価 | 後続証拠文書で具体化する |
| 本文での位置づけ | シナリオベースで安全主張を支える考え方を採用する |

## 6.7 ISO 24089の使い方

ISO 24089:2023は、道路車両のソフトウェア更新エンジニアリングに関する要求事項と推奨事項を、組織レベルおよびプロジェクトレベルで定める文書であり、ソフトウェア更新可能な車両に適用される。

Turingの安全論証では、ISO 24089を、E2E AIモデルおよびADSソフトウェアの更新管理に関する参照枠として用いる。

| **対象** | **ISO 24089の使い方** |
| --- | --- |
| E2E AIモデル更新 | 更新前後で安全主張が維持されることを管理する |
| ADSソフトウェア更新 | 通常走行系、安全監視系、MRC系の更新を管理する |
| 多重系ADS | 通常走行系とMRC系のバージョン不整合を管理する |
| ロールバック | 更新後に問題があった場合の復旧方針を整理する |
| 更新後監視 | 更新後のMRC、フェイルオーバー、異常挙動を監視する |

本書では、具体的な更新手順までは定義しない。モデル更新およびソフトウェア更新は安全論証の対象であり、後続文書で管理方法を具体化する。

## 6.8 ISO/SAE 21434の扱い

ISO/SAE 21434:2021は、道路車両におけるサイバーセキュリティリスク管理のエンジニアリング要求を定める国際規格である。

本書は、サイバーセキュリティケースそのものを主対象としない。ただし、サイバー起因で安全に影響するリスクについては、別途のサイバーセキュリティ論証と接続し、安全論証上の前提条件として扱う。
...
| **ISO** | **本書での主な使い方** | **主に対応する領域** | **本書でやらないこと** |
| --- | --- | --- | --- |
| ISO/TS 5083 | ADS全体の安全論証の骨格 | 最上位安全主張、安全論証全体構造、残存リスク | 初版で詳細な適合表までは作らない |
| ISO 26262 | E/E失陥、ハードウェア失陥、多重系安全の整理 | 多重系ADS、ハードウェア失陥、制動・操舵・電源 | E2E AIの性能不足を26262だけで説明しない |
| ISO 21448 | 別枠での適用は行わない（性能不足の領域はISO/PAS 8800で扱う） | 概念的参照のみ：ODD境界、Known/Unknown Unsafe Scenario、安全側挙動の語彙 | SOTIFプロセス（機能分解、Triggering Condition同定、SOTIF V-cycle）をE2E AIに直接適用しない |
| ISO/PAS 8800 | AI要素の安全保証主張 | E2E AI、安全監視、モデル更新、残存リスク | 特定NN手法の詳細設計規格としては扱わない |
| ISO 34502系 | シナリオベース評価の考え方 | 後続証拠文書、評価シナリオ整理 | 初版で試験条件・合格基準を確定しない |
| ISO 24089 | ソフトウェア・モデル更新管理 | モデル更新、安全論証更新 | 初版でOTA手順や更新運用を詳細化しない |
| ISO/SAE 21434 | サイバーセキュリティとの接続 | 前提条件、隣接文書、運用安全 | 本書をサイバーセキュリティケースにはしない |
